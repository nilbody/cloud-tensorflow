...
parser.add_argument('--mode',
                    type=str,
                    default="train",
                    help="Mode to run, options: train or predict")
...
tf.add_to_collection("outputs", json.dumps({'key': keys.name,
                                            'predict_op1': predict_op1.name,
                                            'predict_op2': predict_op2.name}))
    ...
    if args.mode == "train":
        for i in range(1000):
            for (x, y) in zip(train_X, train_Y):
                _, epoch = sess.run([train_op, global_step],
                                    feed_dict={X: x,
                                               Y: y})
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            predict_x = 10
            predict_y = 20
            predict_result = sess.run(predict_op1,
                                      feed_dict={keys_placeholder: 10,
                                                 X: predict_x,
                                                 Y: predict_y})
            print("x: {}, y: {}, predict result: {}".format(
                predict_x, predict_y, predict_result))
        else:
